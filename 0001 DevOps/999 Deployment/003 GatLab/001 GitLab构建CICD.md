...
--restart always \
-d \
--volume /home/draven/gitlab/config:/etc/gitlab \
--volume /home/draven/gitlab/logs:/var/log/gitlab \
--volume /home/draven/gitlab/data:/var/opt/gitlab \
gitlab/gitlab-ce:latest
...
--non-interactive \
--executor "docker" \
--docker-image "172.16.244.134:5000/ali-maven-docker:3.5.4-jdk-8-alpine" \
--url "http://172.16.244.134/" \
--registration-token "5EJuMWnFEP2yWJmxo57F" \
...
docker run --rm -v /Users/draven/docker/gitlab-runner/config:/etc/gitlab-runner gitlab/gitlab-runner register \
  ...
  --registration-token "9_78Hax7py-8PEQiv8MM" \
  --description "docker-runner" \
  --tag-list "docker" \
  --run-untagged="true" \
  --locked="false" \
...
docker run -idt -v /Users/draven/docker/gitlab-runner/config:/etc/gitlab-runner gitlab/gitlab-runner register \
  --non-interactive \
  --executor "docker" \
  --docker-image '127.0.0.1:5000/ali-maven-docker:3.5.4-jdk-8-alpine' \
  --url "http://127.0.0.1:80" \
